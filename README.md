...
- Управление секретами переменных окружения
...
2. Создайте файл .env в директории infra/ и заполните его переменными окружения:
   ...
   ```
2а. Или используйте Vault. 
Для этого стоит первоочередно зайти в контейнер vault, зарегать root, сохранить ключи и вставить их в .env
В .env должно быть следующее
```
   VAULT_TOKEN=<Ваш токен root юзера>
   UNSEAL_TOKEN=<Ваш токен unseal>
   # Если unseal токенов больше одного, следует их тоже вписать и поменять скрипт unseal.sh
```
Данная версия приложения не будет работать без VAULT
